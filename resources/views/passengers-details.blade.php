    ...
    <body class="bg-light">
        ...
        <div class="success hidden text-center img-fluid">
        	...
        	<div class="content container-sm bg-white rounded-5 p-4">
                ...
                <form action="" id="register-form" name="myForm" class="mb-4 form" enctype="multipart/form-data" method="post">
                	...
                	<div class="mb-3">
                    	<div class="input-group">
                            ...
                            <input type="input" class="form-control" name="phone" id="phone" placeholder="Mobile Number *" pattern="^\s*(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$">
                        </div>
                    </div>
                    <div class="mb-3">
                        <div class="input-group">
                    ...
                    <div class="row mb-3">
                        <div class="input-group">
                            ...
                            <input type="text"  class="form-control" name="name" id="name" placeholder="First Name as per ID/Passport" pattern="^[\w'\-,.][^0-9_!¡?÷?¿/\\+=@#$%ˆ&*(){}|~<>;:[\]]{2,}$">
                        </div>
                    </div>
                    <div class="mb-3">
                    	<input type="text" class="form-control" name="surname" id="surname" placeholder="Surname Name as per ID/Passport" pattern="^[\w'\-,.][^0-9_!¡?÷?¿/\\+=@#$%ˆ&*(){}|~<>;:[\]]{2,}$">
                    </div>
                    <div class="mb-3">
                    	<div class="input-group">
                          	<input type="date" class="form-control border-end-0" id="date" name="date" placeholder="Date of Birth DD/MM/YYYY" pattern="^(?:(?:31(\/|-|\.)(?:0?[13578]|1[02]))\1|(?:(?:29|30)(\/|-|\.)(?:0?[13-9]|1[0-2])\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})$|^(?:29(\/|-|\.)0?2\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\d|2[0-8])(\/|-|\.)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:(?:1[6-9]|[2-9]\d)?\d{2})$">
                    ...
                    <div class="d-grid">
                    	<button type="submit" class="btn btn-success btn-lg rounded-5 py-3 btn-submit">Continue to Payment</button>
                    </div>
        ...
        $("form").on("submit", function (e) {
            ...
            var errordisplay = document.getElementById('form-errors');
            ...
            error: function(error){
                ...
                console.log(error);
                // console.log(`${error.responseJSON.errors} this is the error`)
                var formErr = error.responseJSON.errors;
            
                for(var err in formErr)
                {
                    errordisplay.innerHTML += `<li>${formErr[err][0]}</li>`;
                }
                setTimeout(  function stoperror(){  errordisplay.innerHTML = ''; }, 3000);
